...
Unrecoverable errors will be reported via the `onError` function, if one is
specified in the HOC config object.

```js
import React, { Component } from "react";
import onClickOutside from "react-onclickoutside";
import reportError from "my-error-reporting-service";
